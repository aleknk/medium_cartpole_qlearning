...
def q_learning(agent, n_episodes=1000, show_every=100, verbose=1):
    """
    ...
    for episode in range(int(n_episodes)):
        ...
        # Print logging info if verbose is enabled
        if verbose:
            if episode % int(show_every) == 0 or episode == (int(n_episodes) - 1):
                print(f"    - Episode {episode+1} => Total reward: {episode_reward} || Exploration Prob: {round(agent.exploration_prob,3)}")
...
def main():
    ...
    parser.add_argument("--show_every", type=int, default=1000, help="Frequency of episodes to show progress.")
    parser.add_argument("--save_dir", type=str, default="./results/learn_run_1", help="Directory to save the results.")

    args = parser.parse_args()

    # Extract individual parameters.
    learning_rate, discount_factor, start_exploration_prob, final_exploration_prob, exploration_decay, max_values, min_values, num_bins, n_episodes, show_every, save_dir = parse_args(args)
...
def parse_args(args):
    ...
    if args.params_path:
        ...
        return (params["learning_rate"], params["discount_factor"], 
                ...
                params["num_bins"], params["n_episodes"], params["show_every"],
                params["save_dir"])
        ...
        return (args.learning_rate, args.discount_factor, 
            ...
            args.n_episodes, args.show_every, args.save_dir)
